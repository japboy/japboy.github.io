...
describe("career introduction", () => {
  it("uses a six-second repeat-delay center with less than one second of jitter", () => {
    assert.equal(
  ...
  it("uses the user agent's single preferred language deterministically", () => {
  ...
  it("formats visit duration with at most two human-friendly units", () => {
  ...
  it("accepts supported sentence endings and rejects truncation", () => {
  ...
  it("keeps internal slugs out of the model prompt by converting them to general noun phrases", async () => {
  ...
  it("exposes continuously increasing visit duration without tone categories", async () => {
    const cv = await readCv();
  ...
  it("loops through non-repeating random CV topics after a randomized wait", async () => {
    const cv = await readCv();
    const doubles = [
  ...
  it("waits for visibility before the initial generation", async () => {
    const cv = await readCv();
  ...
  it("preserves the result when hidden during generation and pauses before the repeat timer", async () => {
    const cv = await readCv();
  ...
  it("resumes a paused repeat timer from its remaining duration", async () => {
    const cv = await readCv();
    const double = createConversationDouble(["Completed introduction."]);
    const delays: number[] = [];
  ...
  it("retries an incomplete response once with a smaller deterministic content budget", async () => {
    const cv = await readCv();
    const doubles = [
  ...
  it("fails repeated incomplete responses after the finite retry limit", async () => {
    const cv = await readCv();
  ...
  it("fails empty model output explicitly and keeps cleanup deterministic", async () => {
    const cv = await readCv();
    ...
    assert.match(
      ...
      /empty career introduction/,
    );
    assert.equal(double.getDeleteCount(), 1);
  });

  it("cancels generation after a streamed chunk", async () => {
    const cv = await readCv();
  ...
  it("aborts the randomized wait without leaving a live timer", async () => {
    const cv = await readCv();
    const double = createConversationDouble(["Completed introduction."]);
    const engine = {
      createConversation: async () => double.conversation,
